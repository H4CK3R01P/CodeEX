    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-foreground mb-2">Welcome, {userName}! 👋</h1>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {profiles.map((profile) => (
            <Card
              key={profile.type}
              className="cursor-pointer hover:shadow-2xl transition-all duration-300 border-2 hover:border-primary/50 group"
              onClick={() => onSelect(profile.type)}
            >
              <CardContent className="p-6 text-center">
                <div className={`w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br ${profile.color} flex items-center justify-center text-on-dark group-hover:scale-110 transition-transform duration-300`}>
                  {profile.icon}
                </div>
                <h3 className="text-foreground mb-2">{profile.title}</h3>
