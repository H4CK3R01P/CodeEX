...
import type { UseEmblaCarouselType } from 'embla-carousel-react@8.6.0';
...
function generateDomainContent(domainId: string) {
  ...
  const latestTopics = (subjects || []).slice(0, 4).map((subject, idx) => {
    const chapter = (subject.chapters || [])[idx % (subject.chapters || []).length] || { name: 'Introduction', topics: [] };
    const imageCategory = config.category === 'exam' ? 'exam' : 'coding';
    const subjectKey = subject.id.split('-')[0] || 'default';
    const imageKey = imageCategory === 'exam' ? subject.id.split('-')[0] || 'default' : subjectKey;
    
    return {
      id: `${subject.id}-${idx}`,
      title: chapter.name,
      subject: subject.name,
      thumbnail: domainImages[imageCategory][imageKey as keyof typeof domainImages[typeof imageCategory]] || domainImages[imageCategory].default,
      duration: `${30 + idx * 5} ${terminology.lecture}s`,
      description: `Comprehensive ${terminology.chapter} covering ${chapter.name}`
    };
  });

  // Content sections - different for exam vs coding
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1 bg-white/20 rounded-full h-2 backdrop-blur-sm">
                            <div 
                              ...
                              style={{ width: `${topic.duration.split(' ')[0]}%` }}
                            />
                          </div>
