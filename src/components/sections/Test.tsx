...
} from "../../utils/domainContentGenerator";
...
const domainImages = {
  exam: {
    default:
      "https://images.unsplash.com/photo-1565688695721-2b6f5a880a15?w=800",
...
function generateDomainNews(domainId: string) {
  ...
  return [
    ...
    },
    {
      ...
      image:
        domainImages[config.category].ebook ||
        domainImages[config.category].update,
...
export function Test({ userData }: TestProps) {
  ...
  // Generate domain-specific tests and merge with custom tests
  const allTests = useMemo(() => {
    const generatedTests = generateDomainTests(domainId, 30);
    // Merge custom tests that match the current domain
    const domainCustomTests = customTests.filter(
      (test) => !test.isCustom || test.subject === domainId,
    );
    return [...generatedTests, ...domainCustomTests];
  }, [domainId, customTests]);
  const latestNews = useMemo(
    () => generateDomainNews(domainId),
    [domainId],
  );
  ...
  const testSections: TestSection[] = useMemo(() => {
    ...
    subjects.forEach((subject) => {
      sections.push({
        ...
        tests: allTests.filter(
          (t) => t.subject === subject.name,
        ),
      });
    });
    ...
    sections.push({
      ...
      tests: allTests.filter((t) =>
        t.type.includes(terminology.chapter),
      ),
    });
  ...
  const handleTestCreated = (newTest: any) => {
    // Convert the custom test format to TestItem format
    const testItem: TestItem = {
      ...
      coins: Math.floor(
        (newTest.totalQuestions * newTest.correctMarks) / 10,
      ),
      subject: domainId,
      difficulty:
        newTest.difficulty.charAt(0).toUpperCase() +
        newTest.difficulty.slice(1),
      syllabus: `${newTest.topics.length} topics selected`,
      description: `Custom test on ${newTest.topics.length} selected topics with ${newTest.answerFormat} format`,
      tqs: newTest.totalQuestions,
      alignedToExam: config.name,
      correctMarks: `+${newTest.correctMarks}`,
      incorrectMarks: newTest.incorrectMarks.toString(),
      isCustom: true,
      attempted: false,
    };

    // Add to custom tests list
    setCustomTests((prev) => [...prev, testItem]);

    // View the newly created test
  ...
  if (viewMode === "detail" && selectedTest) {
    return (
      <TestDetail
        test={selectedTest}
        onBack={handleBackToList}
        onStart={() => setViewMode("instructions")}
        userData={userData}
      />
    );
  }
  ...
  // Main test list view
  const filteredSections = testSections.filter(
    (section) =>
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {test.attempted && (
                        <Badge className="ml-2 bg-green-100 text-green-700 flex-shrink-0">
                          Attempted
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      <Badge
                        className={
                          test.difficulty.includes("Hard") ||
                          test.difficulty.includes("Advanced")
                            ? "bg-red-100 text-red-700"
                            : test.difficulty.includes(
                                  "Medium",
                                ) ||
                                test.difficulty.includes(
                                  "Intermediate",
                    ...
                    >
                      {test.attempted ? "Reattempt" : "Start"}{" "}
                      {terminology.test}
                    </Button>
                  </CardContent>
                </Card>
      ...
      {filteredSections.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-gray-900 mb-2">
            No {terminology.test.toLowerCase()}s available
          </h3>
          <p className="text-gray-600">
            {terminology.test}s for this{" "}
            {terminology.subject.toLowerCase()} are coming soon!
          </p>
        </div>
      )}
    </div>
