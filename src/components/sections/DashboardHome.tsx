...
export function DashboardHome({ userData, onNavigate }: DashboardHomeProps) {
  ...
  const stats = [
    { label: (domainData.stats || [])[0]?.label || 'Progress', value: (domainData.stats || [])[0]?.value || '0', icon: <Target className="w-5 h-5" />, gradient: 'from-blue-500 to-cyan-500', bgGradient: 'from-blue-500/10 to-cyan-500/10' },
    ...
    { label: (domainData.stats || [])[1]?.label || 'Rating', value: (domainData.stats || [])[1]?.value || '0', icon: <Trophy className="w-5 h-5" />, gradient: 'from-yellow-500 to-amber-500', bgGradient: 'from-yellow-500/10 to-amber-500/10' },
