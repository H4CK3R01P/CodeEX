...
function generateDomainPracticeContent(domainId: string) {
  ...
  const latestPractice = (subjects || []).slice(0, 4).map((subject, idx) => {
    ...
    const difficulty = config.difficultyLevels[idx % config.difficultyLevels.length] || 'Easy';
    const imageKey = subject.id.split('-')[0] || 'default';
    const imageCategory = config.category === 'exam' ? 'exam' : 'coding';
    
    return {
      id: `${subject.id}-latest-${idx}`,
      title: `${chapter.name} - ${terminology.practice}`,
      questions: 15 + idx * 5,
      duration: `${20 + idx * 5} mins`,
      difficulty,
      ...
      isShort: idx < 2,
      concepts: 5 + idx * 2,
      description: `Practice ${terminology.problem}s on ${chapter.name}`,
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>{practice.concepts} {terminology.topic}s</span>
                          </div>
                        </div>
                      </div>
                    </div>
